...
def prep_receptor(name,inputPath,outputPath):
    ...
    rc("write format mol2 #0 "+outputPath+"/struct_file/receptor.mol2")
    rc("delete H")
    rc("write format pdb #0 "+outputPath+"/struct_file/receptorDelH.pdb")
    rc("surface #0")
    l,w,h,center = calculateWindows(receptor[0].atoms)
    surf = chimera.openModels.list(modelTypes=[chimera.MSMSModel])[0]
    writeDMS(surf, outputPath+"/site_file/receptor.ms")
    rc("close all")
    return l,w,h,center
...
def prep_ligand(name,inputPath,outputPath):
    ...
    rec_charge = estimateFormalCharge(ligand[0].atoms)
    rc("write format mol2 #0 "+outputPath+"/struct_file/ligand.mol2")
    rc("close all")
